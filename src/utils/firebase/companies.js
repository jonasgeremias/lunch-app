...
export async function getCompanieData(id) {
   if (!id) return null;
   return await Firebase.firestore().collection('companies').doc(id).get().then(doc => {
...
export async function setCompanieData(data, adding, user, org) {
   ...
   const ret = await Firebase.firestore().collection('companies').doc(data.companieId).set(item, { merge: true }).then((doc) => {
...
export async function inactiveCompaniesInDB(selectedRowsData, userData) {
   ...
   let ret = await selectedRowsData.map(async (docId) => {
      const query = Firebase.firestore().collection('companies').doc(docId)
